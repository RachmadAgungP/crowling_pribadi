...
def prasyaratan(posisi):
    ...
    for u in range(len(index_h5)-1):
        ...
        # for y in range(len(DATA_kode)):
        #     if len(DATA_kode[y]) > len(DATA_kualif[y]):
        #         DATA_kode[y].append("")
                
        # for y in range(len(DATA_kualif[0])):
        #     if len(DATA_kode[y]) > len(DATA_kualif[0][y]):
        #         DATA_kualif[0][y].append([])
    ...
    for iu in range(len(DATA_kode)):
        print ("lalala",DATA_kode[iu])
        # for ini in range(len(DATA_kode[iu])):
        if len(DATA_kode[iu]) > len(DATA_kualif[iu]):
            DATA_kualif[iu].append([])
        else:
            DATA_kode[iu].append("")
    ...
    for d in range (len(DATA_kualif)):
        if HTML_children_li[d] in DATA_kualif[d]:
            print ("ada",HTML_children_li[d],DATA_kualif[d])
            if HTML_children_li[d] == []:
                continue
            else:
    ...
    return (DATA_Posisi,DATA_kode,DATA_kualif,tag_a_link,HTML_tag_footer,index_h5,HTML_tag_names,artikels)
